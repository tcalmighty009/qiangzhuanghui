...
(function () {
    ...
    var bingo = window.bingo = {
        //主版本号.子版本号.修正版本号
        version: {
            major: 1, minor: 1, rev: 615, toString: function () {
        ...
        isElement: function (obj) {
            var t = obj && (obj.ownerDocument || obj).documentElement;
            return t ? true : false;
        },
        trim: function (str) {
        ...
        toStr: function (p) {
            return this.isNull(p) ? '' : p.toString();
        },
        ...
        proxy: function (owner, fn) {
            return function () {
                return fn && fn.apply(owner, arguments);
            };
        }
    };
...
(function (bingo) {
    ...
    }, _isArrayAttr = function (find) {
        return find.indexOf(']') >= 0 && (find.indexOf('"') < 0 && find.indexOf("'") < 0);
    };
...
(function (bingo) {
    ...
    var _eventDefine = {
        ...
        size: function () {
            return this.__eventList__.length;
        }
    };
...
(function (bingo) {
    ...
    var _extendDefine = function (define, obj) {
        ...
        return fn;
    }, _getProp = function (obj) {
        return obj._bg_prop_ || (obj._bg_prop_ = {});
    };
    ...
    bingo.Class = function () {
        ...
        var define = function () {
            if (arguments[0] != _NewObject_define_String) return define.NewObject.apply(window, arguments);
        };
    ...
    bingo.Class.makeDefine = function (defineName, define) {
        _makeDefine(defineName, define);
    };
...
(function (bingo) {
    ...
    bingo.linq = function (list) {
        return _linqClass.NewObject(list);
    };
...
(function (bingo) {
    ...
    var _ArrayEquals = function (arr1, arr2) {
        ...
        if (!bingo.isArray(arr2) || arr1.length != arr2.length) {
            return false;
...
(function (bingo) {
    ...
    var _inArray = function (element, list) {
        return bingo.inArray(function (item) {
            return bingo.isStringEquals(item, element);
        }, list);
    };
...
(function (bingo) {
    ...
    var _routes = {
        ...
        add: function (name, context) {
            ...
            this.datas = bingo.linq(this.datas).sort(function (item1, item2) {
                return item2.context.priority - item1.context.priority;
            }).toArray()
        },
...
(function (bingo) {
    ...
    var _get = function (cache, key) {
        var item = bingo.linq(cache._datas).where(function () {
            return this.key == key;
        }).first();
        ...
        cache._datas = bingo.linq(cache._datas).where(function () {
            return this.key !== key;
        }).toArray();
    }, _has = function (cache, key) {
        return bingo.linq(cache._datas).where(function () {
            return this.key == key;
        }).contain();
    };
...
(function (bingo) {
    ...
    bingo.extend({
        ...
        getModuleByView: function (view) {
            return view ? view.$getModule() : bingo.defaultModule();
        },
        //定义或获取模块
...
(function (bingo) {
    ...
    bingo.modelOf = function (p) {
        p = bingo.variableOf(p);
        return bingo.isModel(p) ? p.toObject() : p;
    };
...
(function (bingo) {
    ...
    var _ajaxClass = bingo.ajax.ajaxClass = bingo.Class(_ajaxBaseClass, function () {
        ...
        var _loadServer = function (servers, type) {
            ...
            if (servers.isDisposed || (view && view.isDisposed)) {
                _disposeEnd(servers);
                return;
            }
...
(function (bingo, $) {
    ...
    var _compiles = {
        ...
        _makeCommand: function (command, view, node) {
            ...
            if (!bingo.isNullEmpty(opt.tmplUrl)) {
                bingo.tmpl(opt.tmplUrl, view).cacheQurey(true).async(false).success(function (tmpl) {
                    opt.tmpl = tmpl;
                }).get();
            }
    ...
    var _compileClass = bingo.compile.templateClass = bingo.Class(function () {
        ...
        var _traverseChildrenNodes = function (nodes, parentViewnode, view, withDataList, action) {
            //编译一组nodes.
            _compiles.traverseChildrenNodes(nodes, {
                node: null,
                parentViewnode: parentViewnode,
                view: view,
                withData: null,
                action: action,
                withDataList: withDataList
            });
        };
        ...
        this.Define({
            ...
            appendTo: function (jqSelector) {
                this._parentNode = $(jqSelector)[0];
                return this;
            },
            fromNode: function (node) {
                return this.fromJquery(node);
            },
            fromHtml: function (html) {
                return this.fromJquery($.parseHTML(html, true));
        ...
        var _priS = {
            ...
            evalScriptContextFun: function (attr, hasReturn, view, node, withData) {
                ...
                try {
                    ...
                    bingo.trace(e);
                    return attr[cacheName] = function () {
                        return attrValue;
                    };
                }
            }
                ...
                if (!this._isLinked) {
                    ...
                    if (hasSub) {
                        ...
                        bingo.each(tagList, function (item) {
                            ...
                            //检查是否删除
                            view.$subs(function () {
                                return $this._isRemvoe() ? bingo.makeAutoId() : context.$results();
                            }, function (newValue) {
                                if ($this._isRemvoe()) {
                                    $this.dispose();
                                    return;
                    ...
                    if (!hasSub) setTimeout(bingo.proxy(this, function () {
                        this.dispose();
                    }), 1);
                }
            },
...
(function (bingo) {
    ...
    var _filter = {
        ...
        removerFilterString: function (s) {
            ...
            this._filterRegex.lastIndex = 0;
            var str = s.replace(this._filterRegex, function (find, content) {
                if (find.indexOf('||') == 0) return find; else return '';
            });
...
(function (bingo) {
    ...
    var _newItem = function (content, isIf, isEnd, isTag, view, node, isElse, isForeach, role) {
        ...
        if (item.isTag) {
            if (!item.isEnd) {
                ...
                item.fn = function (view, data) {
                    return fnTT(view, data, bingo);
                };//bingo(多版本共存)
            }
        }
        return item;
    ...
    var _compile = function (s, view, node) {
        var list = [],
            ...
            _last = function (len) {
                return (len > 0) ? parents[len - 1].children : list;
            },
            _parent = function (len) {
                return (len > 0) ? parents.pop().children : list;
            };
...
(function (bingo, $) {
    ...
    bingo.factory('$compile', ['$view', function (view) {
        return function () {
            return bingo.compile(view);
        };
    }]);

    bingo.factory('$tmpl', ['$view', function (view) {
        return function (url) {
            return bingo.tmpl(url, view);
        };
    }]);
    ...
    bingo.factory('$ajax', ['$view', function ($view) {
        ...
        fn.syncAll = function (callback) {
            return bingo.ajaxSyncAll(callback, $view);
        };
        return fn;
    }]);
...
(function (bingo) {
    ...
    bingo.command('bg-route-load', function () {
        return ['$location', '$attr', function ($location, $attr) {
            ...
            $attr.$initResults(function (value) {
                bingo.isFunction(value) && $location.onLoaded(function (url) {
                    value.call($location, url);
                });
            });
        }];
    });
